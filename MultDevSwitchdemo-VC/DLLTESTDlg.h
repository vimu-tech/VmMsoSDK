...
#pragma once
#include "..\SharedLibrary\VmMultMsoLib.h"
#include "afxwin.h"
#include "AdvPlot.h"
#include "afxcmn.h"
...
public:
	...
	static void WINAPI UsbDevNoticeAddCallBack(void* ppara, unsigned int dev_id);
	static void WINAPI UsbDevNoticeRemoveCallBack(void* ppara, unsigned int dev_id);
	static void WINAPI DataReadyCallBack(void* ppara, unsigned int dev_id);
	static void WINAPI StreamDataReadyCallBack(void* ppara, unsigned int dev_id, unsigned char channel_index, double* buffer, unsigned int buffer_length,
	...
	static void WINAPI IOReadStateCallBack(void* ppara, unsigned int dev_id, unsigned int state);
// 对话框数据
...
public:
	...
	LRESULT OnStreamDataUpdateMsg(WPARAM wParam, LPARAM lParam);
...
private:
	...
	CEdit m_capture_length_btn;
	unsigned int m_real_length;
	CButton m_check_capture_btn;

	double* m_stream_buffer_ch1;
	double* m_stream_buffer_ch2;
	unsigned int m_stream_buffer_length;
	unsigned int m_stream_sample;
