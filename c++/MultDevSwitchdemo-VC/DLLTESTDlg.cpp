...
#define STREAMDATASUPDATE_MSG  (WM_USER+1557)
...
CDLLTESTDlg::CDLLTESTDlg(CWnd* pParent /*=NULL*/)
	...
	, m_right_range_max(0)
	, m_buffer(NULL)
	, m_stream_buffer_ch1(NULL)
	, m_stream_buffer_ch2(NULL)
	, m_logic_buffer(NULL)
	, m_wave_amp(0)
	, m_wave_bias(0)
	, iosupport(false)
	, m_stream_buffer_length(0)
	, m_stream_sample(0)
{
...
CDLLTESTDlg::~CDLLTESTDlg(void)
{
	...
	if (m_stream_buffer_ch1 != NULL)
	{
		delete[] m_stream_buffer_ch1;
		m_stream_buffer_ch1 = NULL;
	}

	if (m_stream_buffer_ch2 != NULL)
	{
		delete[] m_stream_buffer_ch2;
		m_stream_buffer_ch2 = NULL;
	}
...
void CDLLTESTDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_MODE, m_mode_switch);
	DDX_Control(pDX, IDC_EDIT_LENGTH, m_capture_length_btn);
}
...
BEGIN_MESSAGE_MAP(CDLLTESTDlg, CDialogEx)
	...
	ON_MESSAGE(STREAMDATASUPDATE_MSG, OnStreamDataUpdateMsg)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_MODE, &CDLLTESTDlg::OnCbnSelchangeComboMode)
END_MESSAGE_MAP()
...
//	DetectUsb();
	...
	SetStreamDataReadyCallBack(this, StreamDataReadyCallBack);
...
void WINAPI CDLLTESTDlg::UsbDevNoticeAddCallBack(void* ppara, unsigned int dev_id)
{
	CDLLTESTDlg *pthis=(CDLLTESTDlg*)ppara;
	pthis->PostMessage(USBNOTICE_ADD_MSG, (WPARAM)(1), (LPARAM)(dev_id));
}
...
LRESULT CDLLTESTDlg::OnUsbNoticeAddMsg(WPARAM wParam, LPARAM lParam)
{
	...
	SetOscCaptureMode(m_dev_id, m_mode_switch.GetCurSel());
	...
	sample_num=GetOscSupportSampleRateNum(m_dev_id);
	if(samples!=NULL)
		delete[] samples;
	samples=new unsigned int[sample_num];
	GetOscSupportSampleRates(m_dev_id, samples,sample_num);
	m_sample = GetOscSampleRate(m_dev_id);
	...
	m_capture_length=GetMemoryLength(m_dev_id);
	...
	//////////////////////////////////////////////////////////////////
	if(IsSupportHardTrigger(m_dev_id))
		TRACE("支持硬件触发\n");
	else
		TRACE("不支持硬件触发\n");

	if (IsSupportTriggerForce(m_dev_id))
		TRACE("支持强制触发\n");
	else
		TRACE("不支持强制触发\n");
	...
	unsigned int chufa_mode=GetTriggerMode(m_dev_id);
	...
	unsigned int chufa_style=GetTriggerStyle(m_dev_id);
	...
	unsigned int chufa_source=GetTriggerSource(m_dev_id);
	m_trigger_source.SetCurSel(chufa_source);

	m_trigger_level=GetTriggerLevelmV(m_dev_id);

	//SetPreTriggerPercent(5);
	//////////////////////////////////////////////////////////////////
	m_ch1_ac.SetCheck(FALSE);
	m_ch2_ac.SetCheck(FALSE);
	if(IsSupportAcDc(m_dev_id, 0))
	{
	...
	/////////////////////DDS///////////////////////////
	ddssupport = IsSupportDDSDevice(m_dev_id);
	if(ddssupport)
	{
		int boxing_style_num = GetDDSSupportBoxingStyle(m_dev_id, NULL);
		int *style=new int[boxing_style_num];
		GetDDSSupportBoxingStyle(m_dev_id, style);
		...
		m_wave_bias = 0;
		SetDDSBiasMv(m_dev_id, 0, m_wave_bias);
	...
	/////////////////////IO///////////////////////////
	iosupport = IsSupportIODevice(m_dev_id);
...
void WINAPI CDLLTESTDlg::UsbDevNoticeRemoveCallBack(void* ppara, unsigned int dev_id)
{
	CDLLTESTDlg *pthis=(CDLLTESTDlg*)ppara;
	pthis->PostMessage(USBNOTICE_REMOVE_MSG, (WPARAM)(1), (LPARAM)(dev_id));
}
...
//此处采用发送消息给窗口，这样避免C++的线程，访问窗口信息
void WINAPI CDLLTESTDlg::DataReadyCallBack(void* owner, unsigned int dev_id)
{
...
{
	unsigned int trigger_point = ReadVoltageDatasTriggerPoint(m_dev_id);
	...
	for(int channel=0; channel<2; channel++)
	{
		...
		unsigned int length = ReadVoltageDatas(m_dev_id, channel, m_buffer, m_real_length);
		int outrange = IsVoltageDatasOutRange(m_dev_id, channel);
		...
		//
		if (CalFreq(m_buffer, m_real_length, GetVoltageResolution(m_dev_id, channel), GetOscSampleRate(m_dev_id)))
		{
	...
	{
		unsigned int length =  ReadLogicDatas(m_dev_id, m_logic_buffer, m_real_length);
		TRACE("逻辑分析仪读取数据 %d \n", length);
	}
...
void WINAPI CDLLTESTDlg::StreamDataReadyCallBack(void* ppara, unsigned int dev_id, unsigned char channel_index, double* buffer, unsigned int buffer_length,
	unsigned int failed, unsigned int success, unsigned long long int need_total_sample, unsigned long long int total_sample, unsigned long long int menoryuse)
{
	...
	TRACE("StreamDataReadyCallBack dev_id %d channel_index %d buffer_length %d failed %d success %d need_total_sample %d total_sample %d memoryuse %d\n",
		dev_id, (int)channel_index, buffer_length, failed, success, need_total_sample, total_sample, menoryuse);

	//数据拷贝到自己缓冲区
	if(channel_index==0)
		memcpy(pthis->m_stream_buffer_ch1 + total_sample - buffer_length, buffer, buffer_length * sizeof(double));
	else
		memcpy(pthis->m_stream_buffer_ch2 + total_sample - buffer_length, buffer, buffer_length * sizeof(double));

	//最后一个通道回调完成，准备退出
	if (success)
	{
		pthis->capture_ok_mask |= (0x01 << channel_index);
		if (pthis->capture_ok_mask == pthis->chanenl_mask)
			pthis->capture_ok = true;

		pthis->m_stream_buffer_length = total_sample;
	}
	pthis->PostMessage(STREAMDATASUPDATE_MSG, (WPARAM)(((total_sample - buffer_length) / 1000) * 100 /(need_total_sample /1024)), (LPARAM)(pthis->capture_ok));
}

LRESULT CDLLTESTDlg::OnStreamDataUpdateMsg(WPARAM wParam, LPARAM lParam)
{
	unsigned int percent = wParam;
	unsigned int capture_ok = lParam;

	TRACE("percent %d capture_ok %d \n",percent, capture_ok);
	CString str;
	str.Format("percent %d capture_ok %d \n", percent, capture_ok);
	m_plot.SetRightBottomText(str);
	...
	if (capture_ok)
	{
		...
		m_mode_switch.EnableWindow(TRUE);

		//注意：目前的流模式，数据量很大的时候，软件绘图会慢很多，所有会有卡动
		// 调整垂直刻度和时间，都导致重绘，出现卡动
		//完成后绘图,如果需要改成实时绘图，需要调整
		double timelength = m_stream_buffer_length * 1000000000.0 / m_stream_sample;  //将采集点数换算成时间ns

		lineattribute line0;
		line0.pctName = CH_NAME[0];
		line0.clr = CLR[0];
		line0.buf = NULL;
		line0.length = m_stream_buffer_length;
		m_plot.CreateLine(&line0);
		m_plot.ChangeDatas(CH_NAME[0], m_stream_buffer_ch1, m_stream_buffer_length, 0, timelength);

		lineattribute line1;
		line1.pctName = CH_NAME[1];
		line1.clr = CLR[1];
		line1.buf = NULL;
		line1.length = m_stream_buffer_length;
		m_plot.CreateLine(&line1);
		m_plot.ChangeDatas(CH_NAME[1], m_stream_buffer_ch2, m_stream_buffer_length, 0, timelength);
...
void CDLLTESTDlg::LeftDisplayZoomCtrl(bool start)
{
	...
	{
		SetOscChannelRangemV(m_dev_id, 0, y_min*1000, y_max * 1000);
...
void CDLLTESTDlg::RightDisplayZoomCtrl(bool start)
{
	...
	{
		SetOscChannelRangemV(m_dev_id, 1, y_min * 1000, y_max * 1000);
...
void CDLLTESTDlg::OnCbnSelchangeComboSamples()
{
	if(samples!=NULL)
	{
		...
		SetOscSampleRate(m_dev_id, m_sample);
	}
}
...
void CDLLTESTDlg::OnCbnSelchangeTriggerMode()
{
	...
	if(str==CHUfA_MODE_NAME[0])
		SetTriggerMode(m_dev_id, TRIGGER_MODE_AUTO);
	else if(str==CHUfA_MODE_NAME[1])
		SetTriggerMode(m_dev_id, TRIGGER_MODE_LIANXU);
}
...
void CDLLTESTDlg::OnCbnSelchangeTriggerStyle()
{
	...
	if(str==CHUfA_STYLE_NAME[0])
		SetTriggerStyle(m_dev_id, TRIGGER_STYLE_NONE);
	else if(str==CHUfA_STYLE_NAME[1])
		SetTriggerStyle(m_dev_id, TRIGGER_STYLE_RISE_EDGE);
	else if(str==CHUfA_STYLE_NAME[2])
		SetTriggerStyle(m_dev_id, TRIGGER_STYLE_FALL_EDGE);
	else if(str==CHUfA_STYLE_NAME[3])
		SetTriggerStyle(m_dev_id, TRIGGER_STYLE_EDGE);
}
...
void CDLLTESTDlg::OnCbnSelchangeTriggerSource()
{
	...
	SetTriggerSource(m_dev_id, index);

	//对应IO和逻辑分析仪复用的设备，需要将IO打开并设置为输入
	if ((index>=16)&&(index<=23)&&IsSupportIODevice(m_dev_id))
	{
		IOEnable(m_dev_id, index - 16, 1);
		SetIOInOut(m_dev_id, index - 16, 0);
	}
}
...
void CDLLTESTDlg::OnEnChangeTriggerLevel()
{
	...
	if(GetTriggerSource(m_dev_id) == TRIGGER_SOURCE_CH1)
		SetTriggerLevelmV(m_dev_id, m_trigger_level, (m_left_range_max - m_left_range_min)*1000/10.0/5.0);
	else if (GetTriggerSource(m_dev_id) == TRIGGER_SOURCE_CH2)
		SetTriggerLevelmV(m_dev_id, m_trigger_level, (m_right_range_max - m_right_range_min) * 1000 / 10.0 / 5.0);
}
...
void CDLLTESTDlg::OnBnClickedCheckCapture()
{
	if (m_check_capture_btn.GetCheck())
	{
		...
		m_plot.DestoryLine(CH_NAME[0]);
		m_plot.DestoryLine(CH_NAME[1]);
		...
		m_mode_switch.EnableWindow(FALSE);

		m_plot.SetRightBottomText("");
	}
	else
	{
		AbortCapture(m_dev_id);
...
void CDLLTESTDlg::NextCapture()
{
	if (m_check_capture_btn.GetCheck())
	{
		...
		if (m_mode_switch.GetCurSel())
		{
			m_stream_sample = 1000000;//1M采样率 1秒刷新1次
			uint64_t length = 1024 * 1024 * 10;  //10M

			//缓冲区分配的长一点
			uint64_t mem_length = m_stream_sample /1000 * 1024 * 11;  //10M
			if (m_stream_buffer_ch1 != NULL)
				delete[] m_stream_buffer_ch1;
			m_stream_buffer_ch1 = new double[mem_length];

			if (m_stream_buffer_ch2 != NULL)
				delete[] m_stream_buffer_ch2;
			m_stream_buffer_ch2 = new double[mem_length];

			StreamCapture(m_dev_id, 1, length / 1024, chanenl_mask, m_stream_sample);//1M采样率 1秒刷新1次
		}
		else
		{
			m_real_length = Capture(m_dev_id, m_capture_length, chanenl_mask, 0);
...
{
	SetAcDc(m_dev_id, 0,m_ch1_ac.GetCheck());
}


void CDLLTESTDlg::OnBnClickedCheckAc2()
{
	SetAcDc(m_dev_id, 1,m_ch2_ac.GetCheck());
}
...
void CDLLTESTDlg::OnCbnSelchangeComboBoxing()
{
	if(ddssupport)
	{
		...
		if(BoxingName==BOXING_STYLE_NAME[0])
			SetDDSBoxingStyle(m_dev_id, 0, BX_SINE);
		else if(BoxingName==BOXING_STYLE_NAME[1])
			SetDDSBoxingStyle(m_dev_id, 0, BX_SQUARE);
		else if(BoxingName==BOXING_STYLE_NAME[2])
			SetDDSBoxingStyle(m_dev_id, 0, BX_RAMP);
		else if(BoxingName==BOXING_STYLE_NAME[3])
			SetDDSBoxingStyle(m_dev_id, 0, BX_PULSE);
		else if(BoxingName==BOXING_STYLE_NAME[4])
			SetDDSBoxingStyle(m_dev_id, 0, BX_NOISE);
		else if (BoxingName == BOXING_STYLE_NAME[5])
			SetDDSBoxingStyle(m_dev_id, 0, BX_DC);
		else  if (BoxingName == BOXING_STYLE_NAME[6])
		{
			SetDDSBoxingStyle(m_dev_id, 0, BX_ARB);
			if (GetDDSDepth(m_dev_id) == 8192)
				UpdateDDSArbBuffer(m_dev_id, 0, sinc8192,  sizeof(sinc8192));
			else if (GetDDSDepth(m_dev_id) == 4096)
				UpdateDDSArbBuffer(m_dev_id, 0, sinc4096, sizeof(sinc4096));
		}
	}
}
...
void CDLLTESTDlg::OnEnChangeWaveDutyCycle()
{
	UpdateData(TRUE);
	if(ddssupport)
		SetDDSDutyCycle(m_dev_id, 0, m_wave_duty_cycle);
}
...
{
	if(ddssupport)
		DDSOutputEnable(m_dev_id, 0, !IsDDSOutputEnable(m_dev_id, 0));
}
...
void CDLLTESTDlg::OnBnClickedCheckIo0()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 0, m_enable_io0.GetCheck());
		SetIOInOut(m_dev_id, 0, m_com_inout_io0.GetCurSel());
		SetIOOutState(m_dev_id, 0, m_com_out_io0.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo1()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 1, m_enable_io1.GetCheck());
		SetIOInOut(m_dev_id, 1, m_com_inout_io1.GetCurSel());
		SetIOOutState(m_dev_id, 1, m_com_out_io1.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo2()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 2, m_enable_io2.GetCheck());
		SetIOInOut(m_dev_id, 2, m_com_inout_io2.GetCurSel());
		SetIOOutState(m_dev_id, 2, m_com_out_io2.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo3()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 3, m_enable_io3.GetCheck());
		SetIOInOut(m_dev_id, 3, m_com_inout_io3.GetCurSel());
		SetIOOutState(m_dev_id, 3, m_com_out_io3.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo4()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 4, m_enable_io4.GetCheck());
		SetIOInOut(m_dev_id, 4, m_com_inout_io4.GetCurSel());
		SetIOOutState(m_dev_id, 4, m_com_out_io4.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo5()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 5, m_enable_io5.GetCheck());
		SetIOInOut(m_dev_id, 5, m_com_inout_io5.GetCurSel());
		SetIOOutState(m_dev_id, 5, m_com_out_io5.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo6()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 6, m_enable_io6.GetCheck());
		SetIOInOut(m_dev_id, 6, m_com_inout_io6.GetCurSel());
		SetIOOutState(m_dev_id, 6, m_com_out_io6.GetCurSel());
	}
}
...
void CDLLTESTDlg::OnBnClickedCheckIo7()
{
	if (iosupport)
	{
		IOEnable(m_dev_id, 7, m_enable_io7.GetCheck());
		SetIOInOut(m_dev_id, 7, m_com_inout_io7.GetCurSel());
		SetIOOutState(m_dev_id, 7, m_com_out_io7.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 0))
	{
		SetIOInOut(m_dev_id, 0, m_com_inout_io0.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 1))
	{
		SetIOInOut(m_dev_id, 1, m_com_inout_io1.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 2))
	{
		SetIOInOut(m_dev_id, 2, m_com_inout_io2.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 3))
	{
		SetIOInOut(m_dev_id, 3, m_com_inout_io3.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 4))
	{
		SetIOInOut(m_dev_id, 4, m_com_inout_io4.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 5))
	{
		SetIOInOut(m_dev_id, 5, m_com_inout_io5.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 6))
	{
		SetIOInOut(m_dev_id, 6, m_com_inout_io6.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 7))
	{
		SetIOInOut(m_dev_id, 7, m_com_inout_io7.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 0) && (GetIOInOut(m_dev_id, 0)==1))
	{
		SetIOOutState(m_dev_id, 0, m_com_out_io0.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 1) && (GetIOInOut(m_dev_id, 1) == 1))
	{
		SetIOOutState(m_dev_id, 1, m_com_out_io1.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 2) && (GetIOInOut(m_dev_id, 2) == 1))
	{
		SetIOOutState(m_dev_id, 2, m_com_out_io2.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 3) && (GetIOInOut(m_dev_id, 3) == 1))
	{
		SetIOOutState(m_dev_id, 3, m_com_out_io3.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 4) && (GetIOInOut(m_dev_id, 4) == 1))
	{
		SetIOOutState(m_dev_id, 4, m_com_out_io4.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 5) && (GetIOInOut(m_dev_id, 5) == 1))
	{
		SetIOOutState(m_dev_id, 5, m_com_out_io5.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 6) && (GetIOInOut(m_dev_id, 6) == 1))
	{
		SetIOOutState(m_dev_id, 6, m_com_out_io6.GetCurSel());
	}
}
...
{
	if (iosupport && IsIOEnable(m_dev_id, 7) && (GetIOInOut(m_dev_id, 7) == 1))
	{
		SetIOOutState(m_dev_id, 7, m_com_out_io7.GetCurSel());
	}
}

void WINAPI CDLLTESTDlg::IOReadStateCallBack(void* owner, unsigned int dev_id, unsigned int state)
{
	CDLLTESTDlg* pthis = (CDLLTESTDlg*)owner;
...
void CDLLTESTDlg::OnCbnSelchangeComboMode()
{
	SetOscCaptureMode(m_dev_id, m_mode_switch.GetCurSel());
	...
	if (m_mode_switch.GetCurSel())
	{
		...
		m_samples_combox.EnableWindow(FALSE);
		m_capture_length_btn.EnableWindow(FALSE);
		m_ch1_ac.EnableWindow(FALSE);
		m_ch2_ac.EnableWindow(FALSE);
		m_trigger_mode.EnableWindow(FALSE);
		m_trigger_source.EnableWindow(FALSE);
		m_trigger_style.EnableWindow(FALSE);
	}
	else
	{
		m_check_logic.EnableWindow(TRUE);

		m_samples_combox.EnableWindow(TRUE);
		m_capture_length_btn.EnableWindow(TRUE);
		m_ch1_ac.EnableWindow(TRUE);
		m_ch2_ac.EnableWindow(TRUE);
		m_trigger_mode.EnableWindow(TRUE);
		m_trigger_source.EnableWindow(TRUE);
		m_trigger_style.EnableWindow(TRUE);
	}
