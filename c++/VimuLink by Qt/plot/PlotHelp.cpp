...
void PlotHelp::YAxisValueStr(QString& string_v, Axis::Y_AXIS_DANWEI m_disp_danwei, double m_axis_value, QString& grid)
{
	...
	{
        if (fabs(m_axis_value) >= 1.0)
            string_v = QString("%1/%2").arg(m_axis_value, 0, 'f', 0).arg(grid);
        else
            string_v = QString("%1/%2").arg(m_axis_value, 0, 'f', 3).arg(grid);
	}
}
...
QString PlotHelp::FrequenceStr(double frequence)
{
    ...
    if (fabs(frequence) >= 1000000)
        temp = QString("%1 %2").arg(frequence / 1000000.0, 0, 'f', 3).arg(m_str_mhz);
    else if (fabs(frequence) >= 1000)
        temp = QString("%1 %2").arg(frequence / 1000.0, 0, 'f', 3).arg(m_str_khz);
    else
        temp = QString("%1 %2").arg(frequence, 0, 'f', 3).arg(m_str_hz);
    return temp;
}
