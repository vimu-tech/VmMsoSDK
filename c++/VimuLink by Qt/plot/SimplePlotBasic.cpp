...
//--------------------------------------------------------------------------------
void simplerecordtimeaxis::SetAxisDanwei(Axis::X_AXIS_DANWEI dabwei)
{
    double start, range;
    GetTimeTotalRange(&start, &range);
    double center_time = GetTimeCenter();
    m_axis_danwei = dabwei;
    SetTimeTotalRange(start, range);
    SetTimeCenter(center_time);
}


void simplerecordtimeaxis::SetTimeTotalRange(double start, double range)
{
    m_start_time = start;
    m_total_time = range;
}

void simplerecordtimeaxis::GetTimeTotalRange(double* start, double* range)
{
    *start = m_start_time;
    *range = m_total_time;
}

void simplerecordtimeaxis::SetTimeCenter(double center)
{
    m_center = center;
}

double simplerecordtimeaxis::GetTimeCenter()
{
    return m_center;
}

double simplerecordtimeaxis::GetTimeAdjustIntreval()
{
    const double intreval = getrange() / axis_count_x;

    //消除了最小是1的限制
    double weishu = log10(intreval);
    weishu = (weishu >= 0) ? weishu + 0.5 : weishu - 0.5;
    return pow(10.0, (int)weishu);
}
