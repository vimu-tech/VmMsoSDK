...
CDLLTESTDlg::~CDLLTESTDlg(void)
{
	...
	if (m_buffer_ch1 != NULL)
	{
		delete[]  m_buffer_ch1;
		m_buffer_ch1 = NULL;
	}
	
	if (m_buffer_ch2 != NULL)
	{
		delete[]  m_buffer_ch2;
		m_buffer_ch2 = NULL;
	}
...
LRESULT CDLLTESTDlg::OnUsbNoticeAddMsg(WPARAM wParam, LPARAM lParam)
{
	...
	m_buffer_ch1 = new double[m_capture_length *1024];
	if (m_buffer_ch2 != NULL)
	{
		delete[] m_buffer_ch2;
		m_buffer_ch2 = NULL;
	}
	m_buffer_ch2 = new double[m_capture_length * 1024];
...
LRESULT CDLLTESTDlg::OnDataUpdateMsg(WPARAM wParam, LPARAM lParam)
{
	...
	CString str("");
	if (m_check_ch1.GetCheck() && m_check_ch2.GetCheck())
	{
		int res = CalFreqAndPhaseDif(m_buffer_ch1, GetVoltageResolution(0), m_buffer_ch2, GetVoltageResolution(1),
			m_real_length, GetOscSampleRate(), 10);
		if (res == 1)
		{
			double freq1 = GetFreq1();
			double freq2 = GetFreq2();
			double phasedir = GetPhaseDif();
			str.Format("%0.3f  %0.3f  phase_dif = %0.6f ", freq1, freq2, phasedir);
		}
		else if (res == 2)
		{
			double freq1 = GetFreq1();
			double freq2 = GetFreq2();
			double phasedir = 0;
			str.Format("%0.3f  %0.3f  phase_dif = *** ", freq1, freq2);
		}
		else if (res == 3)
		{
			double freq1 = GetFreq1();
			str.Format("%0.3f  ***  phase_dif = *** ", freq1);
		}
		else if (res == 4)
		{
			double freq2 = GetFreq2();
			str.Format("%0.3f  phase_dif = *** ", freq2);
		}
	}
	else if (m_check_ch1.GetCheck())
	{
		if (CalFreq(m_buffer_ch1, m_real_length, GetVoltageResolution(0), GetOscSampleRate()))
		{
			double freq = GetFreq();
			str.Format("%0.3f  ***  phase_dif = *** ", freq);
		}
	}
	else if (m_check_ch2.GetCheck())
	{
		if (CalFreq(m_buffer_ch2, m_real_length, GetVoltageResolution(1), GetOscSampleRate()))
		{
			double freq = GetFreq();
			str.Format("%0.3f  ***  phase_dif = *** ", freq);
		}
