...
IO_STATE_Z = 2
IO_STATE_PULSE = 3
IO_STATE_DDS1_GATE = 4
IO_STATE_DDS2_GATE = 5
...
def DDS_init(channel_index, out_mode):
    if fIsSupportDDSDevice():
        ...
        if out_mode == DDS_OUT_MODE_CONTINUOUS:
            ...
            fSetDDSFreq(channel_index, 1000)
            fSetDDSBurstStyle(channel_index, 1)  # gate
            fSetDDSTriggerSource(channel_index, DDS_TRIGGER_SOURCE_INTERNAL)
            fSetDDSTriggerSourceIo(channel_index, 4)
            fSetDDSTriggerSourceEnge(channel_index, DDS_ENGES_RISING)
        ...
        print(f"DDS {channel_index} TriggerSourceIo {fGetDDSTriggerSourceIo(channel_index)}!") 
################################################### DDS ###################################################
...
def DevNoticeAddCallBack_func(p):
    ...
    #DDS
    DDS_init(0, DDS_OUT_MODE_BURST)
    DDS_init(1, DDS_OUT_MODE_BURST)
